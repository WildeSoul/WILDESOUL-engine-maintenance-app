...
We executed a comprehensive hyperparameter tuning process across 6 advanced algorithms (including XGBoost, LightGBM, and Random Forest). **MLflow** was used to rigorously log all parameters and track 8 evaluation metrics (Train/Test Accuracy, Precision, Recall, F1). The top-performing algorithm was registered in the Hugging Face Model Hub, guided by a strict quality gate ensuring an F1 score > 0.60. Additionally, **SHAP** explainability was used to verify that the model's decision-making aligned with real-world physical constraints.
